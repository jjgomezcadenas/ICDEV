...
def s12df(run_number, filenames, mode='S1', event_range=NN, print_every=10):
    ...
    (evt, tsp,
     ...
     ns2, cs2, es2, ws2, ts2,
     qs2, xs2, ys2, nsi          ) = create_empty_lists(number_of_lists = 17)
        ...
        with tb.open_file(filename, 'r') as h5in:
            ...
            for event in range(NEVT):
                ...
                if event_number % print_every == 0:
                    print(f"processing in loop = {event_number}, event = {evnt}, tstamp ={tstamp}")
    ...
    pdf = pd.DataFrame(np.array([evt, tsp,  cs1, es1, hs1, ws1, ts1,
                                            cs2, es2,      ws2, ts2,
                                            qs2, xs2, ys2, nsi]).T,
                       columns= ['evt', 'tsp', 'ns1', 'es1', 'hs1', 'ws1', 'ts1',
                                               'ns2', 'es2',        'ws2', 'ts2',
                                               'qs2', 'xs2', 'ys2', 'nsi'])

    nS12 = pd.DataFrame(dict(ns1 = ns1,
                             ns2 = ns2))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mode"   , type=str,            default="S12")
    parser.add_argument("-p"       , type=int, default =  100)
    args     = parser.parse_args(sys.argv[1:])
    ns12, df = s12df(args.r, args.filenames, args.mode, NN, args.p)
    ...
    with tb.open_file(args.o, "r+") as file:
        file.rename_node(file.root.Extra.table, "nS12")
        file.root.Extra.nS12.title              = "nS12"
